//class SiPSMacroDigitizerAlgorithm SimTracker/SiPSMacroDigitizer/src/SiPSMacroDigitizerAlgoithm.cc
...
void SiPSMacroDigitizerAlgorithm::init(const edm::EventSetup& es) {
...
SiPSMacroDigitizerAlgorithm::SiPSMacroDigitizerAlgorithm(const edm::ParameterSet& conf) :
...
{
  LogInfo ("PixelDigitizer ") <<"SiPSMacroDigitizerAlgorithm constructed"
...
std::map<int, SiPSMacroDigitizerAlgorithm::CalParameters, std::less<int> >
SiPSMacroDigitizerAlgorithm::initCal() const {

  using std::cerr;
  using std::cout;
  using std::endl;

  std::map<int, SiPSMacroDigitizerAlgorithm::CalParameters, std::less<int> > calmap;
...
//=========================================================================
SiPSMacroDigitizerAlgorithm::~SiPSMacroDigitizerAlgorithm() {
  LogDebug ("PixelDigitizer")<<"SiPSMacroDigitizerAlgorithm deleted";
}

// Read DynIneff Scale factors from Configuration
SiPSMacroDigitizerAlgorithm::PixelEfficiencies::PixelEfficiencies(const edm::ParameterSet& conf, bool AddPixelInefficiency, int NumberOfBarrelLayers, int NumberOfEndcapDisks) {
  // pixel inefficiency
  ...
  if (AddPixelInefficiency){
    ...
    if (FromConfig) {
      ...
      if (NumberOfBarrelLayers>=5){
        if (NumberOfTotLayers>20){throw cms::Exception("Configuration") <<"SiPSMacroDigitizer was given more layers than it can handle";}
...
void SiPSMacroDigitizerAlgorithm::init_DynIneffDB(const edm::EventSetup& es, const unsigned int& bunchspace){
...
void SiPSMacroDigitizerAlgorithm::PixelEfficiencies::init_from_db(const edm::ESHandle<TrackerGeometry>& geom, const edm::ESHandle<SiPixelDynamicInefficiency>& SiPixelDynamicInefficiency) {
...
bool SiPSMacroDigitizerAlgorithm::PixelEfficiencies::matches(const DetId& detid, const DetId& db_id, const std::vector<uint32_t >& DetIdmasks) {
...
SiPSMacroDigitizerAlgorithm::PixelAging::PixelAging(const edm::ParameterSet& conf, bool AddAging, int NumberOfBarrelLayers, int NumberOfEndcapDisks) {
  // pixel aging
...
//=========================================================================
void SiPSMacroDigitizerAlgorithm::accumulateSimHits(std::vector<PSimHit>::const_iterator inputBegin,
...
//============================================================================
void SiPSMacroDigitizerAlgorithm::calculateInstlumiFactor(PileupMixingContent* puInfo){
...
//============================================================================
void SiPSMacroDigitizerAlgorithm::digitize(const PixelGeomDetUnit* pixdet,
...
#ifdef TP_DEBUG
  LogDebug ("PixelDigitizer") << "[SiPSMacroDigitizerAlgorithm] converted " << digis.size() << " PixelDigis in DetUnit" << detID;
#endif
}
...
void SiPSMacroDigitizerAlgorithm::primary_ionization(const PSimHit& hit, std::vector<EnergyDepositUnit>& ionization_points, CLHEP::HepRandomEngine* engine) const {
...
void SiPSMacroDigitizerAlgorithm::fluctuateEloss(int pid, float particleMomentum,
...
void SiPSMacroDigitizerAlgorithm::drift(const PSimHit& hit,
			              const PixelGeomDetUnit* pixdet,
                                      const GlobalVector& bfield,
				      const TrackerTopology *tTopo,
...
void SiPSMacroDigitizerAlgorithm::induce_signal(const PSimHit& hit,
...
void SiPSMacroDigitizerAlgorithm::make_digis(float thePixelThresholdInE,
                                           uint32_t detID,
					   const PixelGeomDetUnit* pixdet,
                                           std::vector<PixelDigi>& digis,
                                           std::vector<PixelDigiSimLink>& simlinks,
...
void SiPSMacroDigitizerAlgorithm::add_noise(const PixelGeomDetUnit* pixdet,
...
void SiPSMacroDigitizerAlgorithm::pixel_inefficiency(const PixelEfficiencies& eff,
			                           const PixelGeomDetUnit* pixdet,
						   const TrackerTopology *tTopo,
                                                   CLHEP::HepRandomEngine* engine) {
  ...
  if (eff.FromConfig) {
    ...
    if    (pixdet->subDetector()==GeomDetEnumerators::SubDetector::PixelBarrel ||
      ...
      //if (eff.FPixIndex>diskIndex-1){throw cms::Exception("Configuration") <<"SiPSMacroDigitizer is using the wrong efficiency value. index = "
...
float SiPSMacroDigitizerAlgorithm::pixel_aging(const PixelAging& aging,
					     const PixelGeomDetUnit *pixdet,
					     const TrackerTopology *tTopo) const {
...
//float SiPSMacroDigitizerAlgorithm::missCalibrate(const float amp) const {
...
float SiPSMacroDigitizerAlgorithm::missCalibrate(uint32_t detID, const PixelGeomDetUnit* pixdet, int col,int row,
...
LocalVector SiPSMacroDigitizerAlgorithm::DriftDirection(const PixelGeomDetUnit* pixdet,
                                                      const GlobalVector& bfield,
...
void SiPSMacroDigitizerAlgorithm::pixel_inefficiency_db(uint32_t detID) {
...
void SiPSMacroDigitizerAlgorithm::module_killing_conf(uint32_t detID) {
...
//****************************************************************************************************
void SiPSMacroDigitizerAlgorithm::module_killing_DB(uint32_t detID) {

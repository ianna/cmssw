...
TrackerDigiGeometryAnalyzer::analyze( const edm::Event& iEvent, const edm::EventSetup& iSetup )
{
   ...
   for (auto it : pDD->detTypes() ){
     if (std::static_pointer_cast< const PixelGeomDetType >(it)!=nullptr){
       const PixelTopology& p = std::static_pointer_cast< const PixelGeomDetType >(it)->specificTopology();
       PRINT("TrackerDigiGeometryAnalyzer")<<" PIXEL Det " // << it->geographicalId()
                      <<"    Rows    "<<p.nrows() <<"    Columns "<<p.ncolumns()<<'\n';
     }else{
       const StripTopology& p = std::static_pointer_cast< const StripGeomDetType >(it)->specificTopology();

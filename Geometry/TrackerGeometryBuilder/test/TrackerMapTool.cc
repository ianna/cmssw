...
TrackerMapTool::analyze( const edm::Event& iEvent, const edm::EventSetup& iSetup ){
  ...
  for ( ; begin != end; ++begin) {
    ...
    if(subdet==3){nameDet="TIB";name1=" string ";ringno=((*begin)->geographicalId().rawId()>>8)&0x3F;}
    if(subdet==5){nameDet="TOB";name1=" rod ";ringno=((*begin)->geographicalId().rawId()>>8)&0x7F;}
    if(subdet==4){nameDet="TID";name1=" ring ";ringno=((*begin)->geographicalId().rawId()>>8)&0xFF;}
    if(subdet==6){nameDet="TEC";name1=" ring "; }
    name2 = " ";
    if(subdet==6){name2=" petal "; petalno=((*begin)->geographicalId().rawId()>>8)&0x7F;}
    if(subdet==2){name2=" blade "; petalno=((*begin)->geographicalId().rawId()>>8)&0x3F;}
    name3 = " ";
    ...
    if(subdet==3){moduleno=((*begin)->geographicalId().rawId() >>2)&0x3F; }
    if(subdet==5){moduleno=((*begin)->geographicalId().rawId() >>2)&0x3F; }
    if(subdet==4){moduleno=((*begin)->geographicalId().rawId() >>2)&0x1F; }
    if(subdet==6){moduleno=((*begin)->geographicalId().rawId() >>2)&0x3; }
    ...
    if(subdet==2)
      outs << nameDet<<" "<<name0<<" disc " <<layer<<name1<<ring<<name2<<petalno<<" plaquette  "<<moduleno<<name5;
    if(subdet==1 || subdet==3||subdet==4||subdet==5) 
      outs << nameDet<<" "<<name0<<" layer " <<layer<<name1<<ringno<<name2<<name3<<" module  "<<moduleno<<" "<<name4;
    char buffer [20];

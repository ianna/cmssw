...
class RPCGeometry : public TrackingGeometry {
 ...
 public:
  ...
  const std::vector< std::shared_ptr< const RPCChamber >>& chambers() const;

  /// Return a vector of all RPC rolls
  const std::vector< std::shared_ptr< const RPCRoll >>& rolls() const;

  // Return a RPCChamber given its id
  const std::shared_ptr< const RPCChamber > chamber(RPCDetId id) const;
 ...
 private:
  ...
  std::vector< std::shared_ptr< const RPCRoll >> allRolls; // Are not owned by this class; are owned by their chamber.
  std::vector< std::shared_ptr< const RPCChamber >> allChambers; // Are owned by this class.

...
{ 
 ...
 private:
  ...
  std::shared_ptr< const GEMSuperChamber > buildSuperChamber(DDFilteredView& fv, GEMDetId detId) const;

  std::shared_ptr< const GEMChamber > buildChamber(DDFilteredView& fv, GEMDetId detId) const;
  
  std::shared_ptr< const GEMEtaPartition > buildEtaPartition(DDFilteredView& fv, GEMDetId detId) const;
};

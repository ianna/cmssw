...
class CSCGeometry : public TrackingGeometry {
 ...
 public:
  ...
  typedef std::vector< std::shared_ptr< const CSCChamber >> ChamberContainer;
  typedef std::vector< std::shared_ptr< const CSCLayer >> LayerContainer;
  ...
  const std::shared_ptr< const CSCChamber > chamber(CSCDetId id) const;
   ...
   */
  const std::shared_ptr< const CSCChamberSpecs > buildSpecs( int iChamberType,

...
DTGeometryBuilder::buildGeometry(DDFilteredView& fview,
  ...
  while(doChamber) {
    ...
    while(doSL) {
      ...
      // Loop on Layers
      fview.down();
      bool doLayers = fview.subSibling();
      while(doLayers) {
	DTLayer* l = buildLayer(fview, sl, num);
	fview.unCheckNode();
	geom.add(l);

	doLayers = fview.subSibling(); // go to next Layer
      }
...
DTGeometryBuilder::buildLayer(const DDFilteredView& fview,
  ...
  TGeoNode *firstChild = static_cast<TGeoNode*>(fview.volume()->GetNodes()->At(0));
  TGeoNode *currentWire = nullptr;
  TGeoIterator myNext(firstChild->GetMotherVolume());
  myNext.SetType(1);
  int WCounter = 0;
  while((currentWire = myNext.Next())) {
    // if( matches selection )
    // int copyNo = currentWire->GetNumber(); // copy no
    // cout << "    copy no " << copyNo << ", " << fview.noNamespace(currentWire->GetVolume()->GetName()) << "\n";
  // for(int j = 0; j < nws; ++j) {
  //   currentWire = static_cast<TGeoNode*>(fview.node_->GetNodes()->At(j));
  //   if(fview.checkNode(currentWire)) {
  //     // Volume volume = currentWire->GetVolume();
  //     // if(volume->GetShape()->IsA() == TGeoBBox::Class()) {
  //     // 	const TGeoBBox* box = static_cast<const TGeoBBox*>(volume->GetShape());
  //     // 	cout << box->GetDY() << "\n";
    WCounter++;
  }
  int firstWire = firstChild->GetNumber(); // copy no

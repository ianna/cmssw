...
static long algorithm(dd4hep::Detector&,
  ...
  dd4hep::Solid solid = ns.addSolid( name,
                                     dd4hep::Tube(rtmi, rtmx, 0.5*layerDz,
                                                  0., 2._pi) );

  LogDebug("PixelGeom") << "DDPixBarLayerUpgradeAlgo test: "
                        << ns.name()
                        << " Tubs made of "<< genMat
                        << " from 0. to " << convertRadToDeg(2_pi)
                        << " with Rin " << rtmi
                        << " Rout " << rtmx
                        << " ZHalf " << 0.5 * layerDz;

  dd4hep::Volume layer = ns.addVolume(dd4hep::Volume(solid.name(),
                                                     solid,
                                                     ns.material(genMat)));

  // Full Tubes
  solid = ns.addSolid(name+"CoolTube",
                      dd4hep::Tube( 0., coolRadius, 0.5*coolDz, 0., 2_pi));
  ...
  dd4hep::Volume coolTube = ns.addVolume(dd4hep::Volume(solid.name(),
                                                        solid,
                                                        ns.material(tubeMat)));

  // Half Tubes
  solid = ns.addSolid(name+"CoolTubeHalf",
                      dd4hep::Tube(0., coolRadius, 0.5*coolDz , 0, 1_pi));
  ...
  dd4hep::Volume coolTubeHalf = ns.addVolume(dd4hep::Volume(solid.name(),
                                                            solid,
                                                            ns.material(tubeMatHalf)));

  // Full Coolant
  name = ns.objName(parentName);

  solid = ns.addSolid(name+"Coolant",
                      dd4hep::Tube(0., coolRadius - coolThick,
                                   0.5 * coolDz, 0., 2_pi));

  LogDebug("PixelGeom") << "DDPixBarLayerUpgradeAlgo test: " << solid.name()
                        << " Tubs made of " << tubeMat
                        << " from 0 to " << convertRadToDeg(2._pi)
                        << " with Rout " << coolRadius - coolThick
                        << " ZHalf " << 0.5 * coolDz;


  dd4hep::Volume cool = ns.addVolume(dd4hep::Volume(solid.name(),
                                                    solid,
                                                    ns.material(coolMat)));

  coolTube.placeVolume(cool, 1,
                       dd4hep::Transform3D(dd4hep::Rotation3D(),
  ...
  solid = ns.addSolid(name+"CoolantHalf",
                      dd4hep::Tube(0., coolRadius - coolThick,
                                   0.5 * coolDz,
                                   0., 1._pi));
  ...
  dd4hep::Volume coolHalf = ns.addVolume(dd4hep::Volume(solid.name(),
                                                        solid,
                                                        ns.material(coolMatHalf)));

  coolTubeHalf.placeVolume(coolHalf, 1,
                           dd4hep::Transform3D(dd4hep::Rotation3D(),
                                               dd4hep::Position(0., 0., 0.)));
  ...
  for (int i = 1; i < number + 1; i++) {
    ...
    if ((i == 1) || (i == number / 2 + 1)) {
      ...
      tran = dd4hep::Position(rrroffset * cos(phi) - cool1Offset * sin(phi),
                              rrroffset * sin(phi) + cool1Offset * cos(phi),
                              0.);

      layer.placeVolume(coolTube, copyoffset,
                        dd4hep::Transform3D(dd4hep::Rotation3D(),
                                            tran));;

      copyoffset++;
      tran = dd4hep::Position(rrroffset * cos(phi) - cool2Offset * sin(phi),
                              rrroffset * sin(phi) + cool2Offset * cos(phi),
                              0.);
